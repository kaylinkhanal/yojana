...
import IssueModal from "@/components/issueModal/page";
...
export default function App() {
  ...
  const fetchSprintList = async () => {
    const { data } = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/sprints/${selectedProjectId}`
    );
  ...
  const fetchMembers = async () => {
    ...
    setProjectMembers(data.projectList.members);
  };
                      ...
                      return (
                        <>
                          <div
                            ...
                            className="p-4 m-2 bg-white cursor-pointer"
                          >
                            {taskItem.sprintName}
                          </div>
                          {openModal && (
